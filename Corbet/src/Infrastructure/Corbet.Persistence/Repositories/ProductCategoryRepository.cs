...
namespace Corbet.Persistence.Repositories
{
    public class ProductCategoryRepository : BaseRepository<ProductCategory>, IProductCategoryRepo
    {
        ...
        //soft delete
        public async Task<DeleteProductCategoryCommandDto> RemoveCategoryAsync(int categoryId, int? deletedBy)
        {
            ...
            {
                IsProductCateoryExist.DeletedBy = deletedBy;
                IsProductCateoryExist.DeletedDate = DateTime.Now;
        ...
        public Task<bool> CheckCategoryExists(string categoryName)
        {
            ...
            ProductCategory check = _dbContext.ProductCategories.Where(x => x.CategoryName.ToLower() == lowerCaseTax && x.IsDeleted==false).FirstOrDefault();
            if (check != null)
            {
